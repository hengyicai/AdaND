...
class TorchGeneratorWithDialogEvalAgent(TorchGeneratorAgent):
    ...
    def report(self):
        ...
        if not self.model.training:
            ...
            m['total_metric'] = \
                (-base.get('ppl' if 'ppl' not in self.metrics_exclude_from_total_metric else 'NON_EXIST',
                           0) * 0.25) / 100 + \
                ...
                m.get(
                    'response_length' if 'response_length' not in self.metrics_exclude_from_total_metric else 'NON_EXIST',
