...
from parlai.utils.misc import NEAR_INF, NEAR_INF_FP16
...
class RNNEncoder(nn.Module):
    ...
    def forward(self, xs):
        ...
        # Warning: temporarily set to be False for the convenience of baseline CVAE_GAN in auto data manipulation
        packed = False
...
class OutputLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.padding_idx >= 0:
            scores[:, :, self.padding_idx] = -NEAR_INF_FP16 if scores.dtype == torch.float16 else -NEAR_INF
    ...
    def forward(self, xes, hidden, attn_params):
        ...
        if self.attention == 'local':
            ...
            if attn_mask is not None:
                ...
                attn_w_premask.masked_fill_((~attn_mask), -NEAR_INF_FP16 if hid.dtype == torch.float16 else -NEAR_INF)
