...
def sentence_greedy_score(tokens1, tokens2, w2v):
    ...
    for tok in tokens2:
        if tok in w2v.stoi:
            Y = np.hstack((Y, (w2v[tok].numpy().reshape((dim, 1)))))
            y_count += 1
...
def sentence_extrema_score(tokens1, tokens2, w2v):
    X = []
    for tok in tokens1:
        if tok in w2v.stoi:
            X.append(w2v[tok].numpy())
    Y = []
    for tok in tokens2:
        if tok in w2v.stoi:
            Y.append(w2v[tok].numpy())
...
def sentence_average_score(tokens1, tokens2, w2v):
    dim = w2v.vectors.shape[1]
    X = np.zeros((dim,))
    for tok in tokens1:
        if tok in w2v.stoi:
